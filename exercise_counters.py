...
class ExerciseCounter:
    """Basic exercise counter with angle-based detection and real-time JSON export"""
    ...
    def export_live_data(self, exercise_type, keypoints):
        ...
        try:
            ...
            if isinstance(keypoints, np.ndarray):
                skeleton_coords = keypoints.tolist()
            else:
                # If it's already a list or other type, use as-is
                skeleton_coords = list(keypoints) if hasattr(keypoints, '__iter__') else []
            
            # Get current stage (handle leg exercises separately)
            current_stage = self.stage
            if exercise_type in self.leg_exercises:
                # For leg exercises, use the stage that just completed a rep or default
                current_stage = "both"
                if self.leg_stages['left'] == "up":
                    current_stage = "left_up"
                elif self.leg_stages['right'] == "up":
                    current_stage = "right_up"
                elif self.leg_stages['left'] == "down":
                    current_stage = "left_down"
                elif self.leg_stages['right'] == "down":
                    current_stage = "right_down"
            ...
            live_data = {
                ...
                "current_stage": current_stage if current_stage is not None else "unknown",
                "device_id": "cam_01",  # Hardcoded device ID
                "skeleton_coords": skeleton_coords  # All keypoints in list format
            }
            
            # Write to JSON file (overwrite mode to ensure backend reads latest frame)
            with open('live_stream_data.json', 'w') as json_file:
                json.dump(live_data, json_file, indent=2)
                
            # Optional: Debug logging (can be disabled in production)
            # print(f"Exported data for {exercise_type}: {self.counter} reps, stage: {current_stage}")
            
        except IOError as e:
            print(f"Warning: Failed to write live data to JSON file: {e}")
            # Continue execution - don't crash the application
        except Exception as e:
            print(f"Warning: Error in export_live_data: {e}")
            # Continue execution - don't crash the application
    
    def count_exercise(self, keypoints, exercise_type):
        """Generic exercise counting function with real-time export"""
        try:
            ...
            if left_angle is None or right_angle is None:
                # Still export data even if angles are None
                self.export_live_data(exercise_type, keypoints)
                return None
            
            # Handle leg exercises differently
            if exercise_type in self.leg_exercises:
                result = self.count_leg_exercise(left_angle, right_angle, config)
                # Export data for leg exercises
                self.export_live_data(exercise_type, keypoints)
                return result
            ...
            # Still attempt to export data even on error
            try:
                self.export_live_data(exercise_type, keypoints)
            except:
                pass
